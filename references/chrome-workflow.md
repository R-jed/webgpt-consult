...
- `rollover_branch`: same workstream, but the registered conversation has context-pressure evidence and can branch from the stable branch base;
- `rollover_fresh`: same workstream, context-pressure evidence exists, but a reliable Web branch cannot be created.
...
If a stored URL fails to load or is not the intended conversation, retire it and open a fresh conversation. Carry forward only verified local continuity state.
...
Treat rollover as required when fresh UI evidence shows a context/conversation-length rejection, or when the user explicitly requests a continuity-preserving rollover. A demonstrable loss of material earlier decisions may also justify rollover when continuing in place would make the consultation unreliable.
...
The plan returns the current parent conversation URL, stable `branch_base_task_id`, last successful task ID, and next rollover index.

Create and validate `CONTINUITY_CAPSULE_V1` before touching the Branch action. The capsule must be cumulative from the stable base through the latest successful consultation and must pass `scripts/continuity_capsule.py`.
...
7. Send the rollover packet containing the validated continuity capsule plus the current delta and evidence.

Branch from the stable base, not from the latest message. Branching from the latest message retains the long tail and does not reduce context pressure.

### Fallback path: rollover_fresh

If the Branch action is unavailable, the branch-base turn cannot be uniquely verified, the new URL is not created, or the inherited baseline is wrong, stop the branch attempt. Create a completely fresh ChatGPT conversation and send the same validated continuity capsule as a standalone restorable baseline.

Re-upload any current artifact whose contents matter and are not faithfully represented in the capsule. Do not keep sending into the overfull parent conversation.
...
For a rollover, add `--rollover`, the previous parent URL, and the validated capsule SHA-256. The registry keeps the original `branch_base_task_id`, increments rollover count, and preserves recent parent conversation URLs for auditability.

Do not change the branch base to the newest response after each rollover. Reusing the original compact base plus a cumulative capsule keeps future branches bounded instead of allowing inherited history to grow again.

The registry is local operational state and must never be uploaded to ChatGPT.
