...
def _is_outside_fences(content: str, pos: int) -> bool:
    """Check if ``pos`` is outside any fenced code block.

    Supports backtick fences (```) and tilde fences (~~~) with:
    - up to 3 leading spaces on the delimiter line
    - info strings after the opening delimiter
    - closing fence must use the same character and be >= opening length

    State machine: builds line starts, then checks each line for fence
    delimiters at known positions.
    """
    import re as _re

    FENCE_RE = _re.compile(r"^( {0,3})(`{3,}|~{3,})", _re.MULTILINE)
    ...
    open_fences: list[tuple[str, int]] = []  # (char, length)

    for ls in line_starts:
        if ls >= pos:
            break
        m = FENCE_RE.match(content, ls)
        if not m:
            continue
        captured_spaces = len(m.group(1))
        actual_leading = m.group(1).count(" ")
        if actual_leading > captured_spaces:
            continue  # 4+ leading spaces → code block, not fence

        char = m.group(2)[0]
        length = len(m.group(2))

        if not open_fences:
            open_fences.append((char, length))
        else:
            top_char, top_len = open_fences[-1]
            if char == top_char and length >= top_len:
                open_fences.pop()
            else:
                open_fences.append((char, length))

    return not open_fences
...
def split_latest_assistant(content: str) -> str:
    ...
    for marker in ASSISTANT_MARKERS:
        ...
        while True:
            ...
            if _is_outside_fences(content, found):
    ...
    for next_marker in ASSISTANT_MARKERS + USER_MARKERS:
        # Skip position 0 – that's the start of our own extracted reply.
        search_start = 1
        while True:
            pos = reply.find(next_marker, search_start)
            if pos == -1:
                break
            # Only count markers outside fenced code blocks.
            if _is_outside_fences(reply, pos):
                next_positions.append(pos)
                break
            search_start = pos + len(next_marker)
    if next_positions:
