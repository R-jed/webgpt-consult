...
def compact_excerpt(text: str, start: int, end: int) -> str:
    """Build a redacted excerpt: key name + location, never the secret value."""
    left = max(0, start - 12)
...
def _redact_finding(text: str, finding: dict) -> dict:
    """Strip credential values from a finding's excerpt.

    For key=value patterns, keep the key and redact the value.
    For raw tokens (AKIA..., sk-..., etc.), redact the entire match.
    """
    if finding["severity"] != "high":
        return finding
    matched = text[finding["start"]:finding["end"]]
    # Keep only the key portion if there's a separator (= or :)
    parts = re.split(r"[:=]", matched, maxsplit=1)
    if len(parts) > 1 and parts[0].strip():
        key_part = parts[0].strip()
        return {**finding, "excerpt": f"{key_part}=<REDACTED>"}
    # Raw token (no key=value structure) → redact entire match
    ...
    for severity, patterns in (("high", HIGH_PATTERNS), ("warn", WARN_PATTERNS)):
        for name, pattern in patterns:
            for match in pattern.finditer(text):
                ...
                # Redact credential values – never expose raw secrets.
                if severity == "high":
                    finding = _redact_finding(text, finding)
